...
export const getFollowers = async (userId: string) => {
  ...
  const { data } = await apiClient.get<PaginatedResponse<User>>(`/users/${userId}/followers`);
...
export const getFollowings = async (userId: string) => {
  const { data } = await apiClient.get<PaginatedResponse<User>>(`/users/${userId}/followings`);
  return data.data; // Возвращаем только массив пользователей
};
...
export const followUser = async (followingId: number) => {
    ...
    const { data } = await apiClient.post('/follows', { following_id: followingId });
    return data;
  };
  ...
  export const getMyFollowings = async () => {
    const { data } = await apiClient.get<PaginatedResponse<User>>('/me/followings');
    return data.data;
  };